...
Again, taking $t \xrightarrow{} T_0$, and since we should know $ P_c(T_0, T_{1Y}) = \mathbb{E}_t^{Q_f} \left\[D_c(T_0,T_{1Y}) \right\]$ from the previous step, the only unknown is $ P_c(T_0, T_{2Y})$.
