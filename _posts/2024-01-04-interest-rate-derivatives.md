...
- **Forward curve**: It's the implied rates by current zero rates for periods of time in the future. For example, the rate of a 1-year zero-coupon bond that starts in one year and ends in two. A forward rate between $T_1$ and $T_2$ is defined as $f(T_1,T_2) = z(t,T_2) + (z(t,T_2) - z(t,T_1)) \cdot \frac{\tau(t,T_1)}{{\tau(T_1,T_2)}$
